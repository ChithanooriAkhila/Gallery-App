...
class Gallery extends Component {
  state = {imgUrl: imagesList[0].imageUrl}
  ...
  render() {
    ...
    const activeItem = imagesList.filter(
      imageDetails => imageDetails.imageUrl === imgUrl,
    )
    const {imageAltText} = activeItem[0]
          ...
          <ul className="thumbnails-container">
            {imagesList.map(imageDetails => (
              <ThumbnailItem
                ...
                activeThumbnail={imageDetails.imageUrl === imgUrl}
              />
            ))}
          </ul>
        </div>
